import { handle, randomId } from "@/lib/api";
import { store } from "@/lib/store";
import { AppError } from "@/lib/service";
import { fromIstLocal } from "@/lib/time";

/** Riya creates a trip with just her own name. Friends add themselves from the shared link. */
export async function POST(req: Request) {
  return handle(async () => {
    const body = await req.json();
    const name = String(body.name ?? "").trim().slice(0, 60);
    const month = String(body.month ?? "");
    const yourName = String(body.yourName ?? "").trim().slice(0, 30);
    const yourPhone = String(body.yourPhone ?? "").trim().slice(0, 20);
    const size = Number(body.groupSize);
    const expected = Number.isInteger(size) && size >= 2 && size <= 20 ? size : null;
    if (!name) throw new AppError(400, "Give the trip a name.");
    if (!/^\d{4}-\d{2}$/.test(month)) throw new AppError(400, "Pick a target month.");
    if (!yourName) throw new AppError(400, "Add your name.");
    const deadline = fromIstLocal(String(body.deadline ?? ""));
    if (Number.isNaN(deadline.getTime())) throw new AppError(400, "Pick a deadline for answers.");
    if (deadline.getTime() < Date.now()) throw new AppError(400, "The deadline should be in the future.");

    const base = name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 24) || "trip";
    const slug = `${base}-${randomId(5)}`;
    const adminKey = randomId(16);
    const b = await store.createTrip({
      slug,
      admin_key: adminKey,
      name,
      target_month: `${month}-01`,
      deadline: deadline.toISOString(),
      is_demo: false,
      expected_size: expected,
      members: [{ name: yourName, phone: yourPhone, is_coordinator: true }],
    });
    return { slug, adminKey, memberId: b.members[0].id };
  });
}
